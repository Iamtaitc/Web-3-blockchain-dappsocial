...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    constructor(address _nftMedia, address _feeRecipient) Ownable(msg.sender) {
    ...
    function listNFT(uint256 _tokenId, uint256 _price) external nonReentrant {
        ...
        require(
            nftMedia.getApproved(_tokenId) == address(this),
            "Not approved"
        );
    ...
    function buyNFT(uint256 _tokenId) external payable nonReentrant {
        ...
        (address creator, uint256 royaltyPercent) = nftMedia.getRoyaltyInfo(
            _tokenId
        );
        ...
        require(sellerTransferSuccess, "Failed to send ETH to seller");

        // Transfer ETH to fee recipient if necessary
        if (marketFee > 0) {
            (bool feeTransferSuccess, ) = payable(feeRecipient).call{
                value: marketFee
            }("");
            require(feeTransferSuccess, "Failed to send ETH to fee recipient");
        }

        // Transfer ETH to creator for royalties if necessary
        if (royaltyFee > 0) {
            (bool royaltyTransferSuccess, ) = payable(creator).call{
                value: royaltyFee
            }("");
            require(royaltyTransferSuccess, "Failed to send ETH to creator");
        }
